...
```
As the last result show, this is the best model we have made so far from now. And also let's do some confidence analysis. The confidence interval of all the feature in the Table1.
```{r, echo=FALSE}
...
========
          ...
          * Table1   Confidence Intervals of model lm(mpg ~ wt + cyl + hp + disp + hp:disp)
